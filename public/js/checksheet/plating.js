...
class PlatingCreate {
    ...
    initPersistentFields() {
        ...
        $(document).on("change", 'select[name="line"], #lineSelect', function () {
            ...
            if (val) {
                ...
                try { localStorage.setItem("qc_last_line_plating", val); } catch (e) {}
            }
        });
    ...
    restorePersistentFields() {
        ...
        const savedLine = this.selectedLine || (function() {
            try { return localStorage.getItem("qc_last_line_plating"); } catch (e) { return null; }
        })();
                ...
                success: (res) => {
                    if (res.success) {
                        ...
                        if (isModalOpen) {
                            ...
                            _this.lastSavedQR = savedQR;
                            _this.postSaveCooldown = true;
                            clearTimeout(_this.postSaveCooldownTimer);
                            _this.postSaveCooldownTimer = setTimeout(() => {
                                _this.postSaveCooldown = false;
                                _this.lastSavedQR = null;
                            }, 5000);
