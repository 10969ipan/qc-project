...
class PaintingIndex {
        ...
        $(document).on("click", ".btn-edit-modal", (e) => {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (window.initPaintingEdit) window.initPaintingEdit();
                },
                error: (xhr) => {
    ...
    initQRDetail() {
        ...
        // Painting Pasang: PartCode|LotId|UniqueCode|DateOpsShift|Qty|JIG-xxx  (6 segments, last starts with JIG-)
        ...
        // Painting Cabut: PartCode|PO|QtyOrig|LotCabut|QtySplit|CBT-xxx  (6 segments, last starts with CBT-)
...
class PaintingCreate {
        ...
        try {
            ...
            if (parts.length >= 5) {
                ...
                Swal.fire({
                    ...
                    text: "Data QR tidak sesuai standar (" + decodedText + ")",
                    confirmButtonText: "Mengerti",
                }).then(() => {
                    this.isProcessingScan = false;
                });
    ...
    processFillQR(decodedText, parts) {
        try {
            ...
            // Auto-fill no_lot, painting_date, painting_shift if lotCabut is available
            if (lotCabut) {
                ...
                if (lotCabut.includes("-")) {
                    ...
                    if (lotParts.length === 3) {
                        ...
                        if (dateStr.length === 8) {
                            ...
                            $("#paintingDateInput").val(formattedDate);
                        }
                        const shiftStr = lotParts[2]; // e.g. 1
                        if (shiftStr === "1" || shiftStr === "2" || shiftStr === "3") {
                            $("#paintingShiftInput").val(shiftStr);
                        }
                    }
    ...
    updateJudgment() {
        ...
        const isPaintingChecked = $("#checkOK").is(":checked");
    ...
    initFormSubmit() {
        $(document).on("submit", "#checksheetForm", (e) => {
            ...
            console.log("Submitting Painting Form...");
    ...
    initHardwareScanner() {
        ...
        const processScan = (raw) => {
            raw = (raw || "").trim();
            console.log("Hardware Scan Triggered (Painting):", raw);
...
window.initPaintingIndex = (config) => new PaintingIndex(config);
window.initPaintingCreate = (config) => new PaintingCreate(config);
window.initPaintingEdit = () => new PaintingEdit();
