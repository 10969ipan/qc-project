# 📝 Rencana Optimasi Performa & Memori Modul Checksheet Cross-Cut Painting (Sub-Assy Pattern & Image Compressor)

**Lokasi File Dokumen**: `docs/plan_crosscut_painting_update_performa.md`  
**Tanggal**: 22 Agustus 2026  
**Target Modul**: Checksheet Cross-Cut Painting (`cross_cut_painting`)

---

## 1. Pendahuluan & Tujuan Optimasi

Dokumen ini berisi detail rencana pengodean (*Detailed Code Implementation Plan*) untuk mengoptimasi kinerja backend, kueri database, serta **konsumsi RAM & disk memori server pada transaksi upload foto** modul **Checksheet Cross-Cut Painting**, dengan mengadopsi pola pengodean Sub-Assy dan modul `ImageCompressor`.

Target optimasi:
* Menurunkan waktu respon halaman index & paginasi dari **> 100 ms / beberapa detik** menjadi **< 40 ms**.
* Mengeliminasi **Subquery Full Table Scan Lintas Plant** pada tabel `cross_cut_painting_checksheets`.
...
### 2.1. Controller Layer: `app/Http/Controllers/CrossCutPaintingChecksheetController.php`
...
$customers = \Illuminate\Support\Facades\Cache::remember("crosscut_painting_filter_cust_{$plantId}", 1800, function () use ($plantId) {
...
$initials = \Illuminate\Support\Facades\Cache::remember("crosscut_painting_filter_init_{$plantId}", 1800, function () use ($plantId) {
    return CrossCutPaintingChecksheet::where('plant_id', $plantId)
...
### 2.2. Service Layer: `app/Services/CrossCutPaintingChecksheetService.php`
...
1. **Uji Kecepatan Respon Index Page**:
   Mengukur waktu eksekusi `CrossCutPaintingChecksheetController::index()` via script CLI & browser DevTools.
2. **Uji Kompresi Foto**:
...
4. **Uji Auto-Reset Cache**:
   Memastikan saat checksheet Cross-Cut Painting baru diinput, pilihan di dropdown filter langsung ter-update secara otomatis.
