...
class IncomingMaterialService extends BaseService
{
    ...
    {
        $query = IncomingMaterial::with('item')->orderBy('date', 'desc')->orderBy('created_at', 'desc');
    ...
    public function createChecksheet(array $data): array
    {
        ...
        try {
            ...
            $checksheet = IncomingMaterial::create([
                ...
                'komper_karung_kg' => $data['komper_karung_kg'],
                'sampling_size_karung_kg' => $data['sampling_size_karung_kg'],
            ...
            DB::commit();
            \Illuminate\Support\Facades\Cache::forget("incoming_materials_filters_" . md5(json_encode([$checksheet->plant_id])));
            ...
            DB::rollBack();
            Log::error('Gagal membuat checksheet Incoming Material', ['error' => $e->getMessage()]);
            throw $e;
        }
    }

    public function updateChecksheet(int $id, array $data): IncomingMaterial
    {
        DB::beginTransaction();
        try {
            ...
            DB::rollBack();
            Log::error('Gagal memperbarui checksheet Incoming Material', ['error' => $e->getMessage()]);
            throw $e;
        }
    }
    ...
    public function updateApprovalStatus(int $id, array $data): IncomingMaterial
    {
        DB::beginTransaction();
        try {
            $checksheet = IncomingMaterial::findOrFail($id);
