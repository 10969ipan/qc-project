...
class CrossCutPaintingChecksheetService extends BaseService
{
    ...
    {
        $query = CrossCutPaintingChecksheet::with('item')->orderBy('qc_datetime', 'desc')->orderBy('created_at', 'desc');
    ...
    /**
     * Create new cross cut painting checksheet
     * 
     * @param array $data
     * @return CrossCutPaintingChecksheet
     */
    public function createChecksheet(array $data): CrossCutPaintingChecksheet
    {
        DB::beginTransaction();
        try {
            $imagePath = null;
            ...
            $checksheet = CrossCutPaintingChecksheet::create([
                ...
                'pencil_scratch' => $data['pencil_scratch'] ?? null,
                'tap_test' => $data['tap_test'] ?? null,
            ...
            Log::info('Checksheet Cross Cut Painting berhasil dibuat', [
                'user_id' => auth()->id(),
            ...
            DB::rollBack();
            Log::error('Gagal membuat checksheet Cross Cut Painting', [
                'user_id' => auth()->id(),
                'error' => $e->getMessage(),
                'data' => $data
            ]);
    ...
    /**
     * Update cross cut painting checksheet
     * 
     * @param int $id
     * @param array $data
     * @return CrossCutPaintingChecksheet
     */
    public function updateChecksheet(int $id, array $data): CrossCutPaintingChecksheet
    {
        DB::beginTransaction();
        try {
            ...
            Log::info('Checksheet Cross Cut Painting berhasil diperbarui', [
                'user_id' => auth()->id(),
                'checksheet_id' => $checksheet->id,
                'plant_id' => $checksheet->plant_id
            ]);
            ...
            DB::rollBack();
            Log::error('Gagal memperbarui checksheet Cross Cut Painting', [
                'user_id' => auth()->id(),
                'checksheet_id' => $id,
                'error' => $e->getMessage()
            ]);
    ...
    /**
     * Delete cross cut painting checksheet
     * 
     * @param int $id
     * @return bool
     */
    public function deleteChecksheet(int $id): bool
    {
        ...
        try {
            ...
            Log::info('Checksheet Cross Cut Painting berhasil dihapus', [
                'user_id' => auth()->id(),
                'checksheet_id' => $id
            ]);
            ...
            DB::rollBack();
            Log::error('Gagal menghapus checksheet Cross Cut Painting', [
                'user_id' => auth()->id(),
                'checksheet_id' => $id,
                'error' => $e->getMessage()
            ]);
     ...
     * @return CrossCutPaintingChecksheet
     */
    public function updateApprovalStatus(int $id, array $data): CrossCutPaintingChecksheet
    {
        DB::beginTransaction();
        try {
            $checksheet = CrossCutPaintingChecksheet::findOrFail($id);
     ...
     * 
     * @param CrossCutPaintingChecksheet $checksheet
     * @param string $field
     ...
     * @return void
     */
    private function updateLevel(CrossCutPaintingChecksheet $checksheet, $field, $dateField, $status, $user): void
    {
        if ($status === 'Approved') {
     ...
     * @return CrossCutPaintingChecksheet
     */
    public function singleApprove(int $id, string $type, array $data): CrossCutPaintingChecksheet
    {
        DB::beginTransaction();
        try {
            $query = CrossCutPaintingChecksheet::query();
            ...
            // Check if rejected
            $field = $this->getApprovalField($type);
            $wasRejected = ($checksheet->$field === 'REJECTED');
            if ($wasRejected) {
    ...
    /**
     * Reject checksheet at a specific level
     * 
     * @param int $id
     * @param string $type
     ...
     * @return CrossCutPaintingChecksheet
     */
    public function rejectChecksheet(int $id, string $type, string $remarks): CrossCutPaintingChecksheet
    {
        DB::beginTransaction();
        try {
            $checksheet = CrossCutPaintingChecksheet::findOrFail($id);
            ...
            $checksheet->$field = 'REJECTED';
            $checksheet->rejection_remarks = $remarks;
            $checksheet->approval_status = 'Rejected';
            $checksheet->save();
