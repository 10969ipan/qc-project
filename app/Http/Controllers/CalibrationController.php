...
class CalibrationController extends Controller
{
    ...
    public function toolsEdit($id, Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($tool->tanggal_beli) {
                $tool->tanggal_beli_formatted = \Carbon\Carbon::parse($tool->tanggal_beli)->format('Y-m-d');
            }

            $tool->schedules->each(function ($sch) {
                $sch->schedule_date_formatted = \Carbon\Carbon::parse($sch->schedule_date)->format('Y-m-d');
            });
    ...
    public function updateProblemJudgment(Request $request, $id)
    {
        ...
        if ($request->judgment_status === 'OK') {
            ...
            $message = 'Judgment OK berhasil disimpan. Alat kembali ACTIVE.';
        } else {
            // If NG, soft-delete the tool to remove from master data & schedules
            $tool->schedules()->delete();
            $tool->delete();
            $message = 'Judgment NG berhasil disimpan. Alat telah dihapus.';
        }
