...
class StandardPerformanceTestController extends Controller
{
    ...
    public function storeThickness(Request $request)
    {
        ...
        if ($request->boolean('is_trial')) {
            $reportTrial = DurabilityThicknessReport::create([
                ...
                'tanggal_cek_porecount' => $request->filled('actual_porecount') && $request->actual_porecount !== '-' ? ($request->tanggal_cek_porecount ?? $request->tanggal_test ?? now()->toDateString()) : null,
    ...
    public function updateThickness(Request $request, $id)
    {
        ...
        if ($testType === 'porecount') {
            if ($request->has('tanggal_test') && !$request->has('tanggal_cek_porecount')) {
                $request->merge(['tanggal_cek_porecount' => $request->tanggal_test]);
            }
            if ($request->has('tanggal_cek') && !$request->has('tanggal_cek_porecount')) {
                $request->merge(['tanggal_cek_porecount' => $request->tanggal_cek]);
            }
        }
