...
class StorePaintingChecksheetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'unique_code_id' => [
                'nullable',
                'string',
                \Illuminate\Validation\Rule::unique('painting_checksheets', 'unique_code_id')->where(function ($query) {
            ...
            'injection_date' => $this->boolean('is_scanned') ? 'nullable|date' : 'required|date',
            'injection_shift' => $this->boolean('is_scanned') ? 'nullable|string' : 'required|string',
            'injection_initials' => $this->boolean('is_scanned') ? 'nullable|string' : 'required|string',
            'painting_date' => $this->boolean('is_scanned') ? 'nullable|date' : 'required|date',
            'painting_shift' => $this->boolean('is_scanned') ? 'nullable|string' : 'required|string',
    ...
    public function messages(): array
    {
        return [
            ...
            'injection_date.required' => 'Tanggal Lot ID wajib diisi.',
            'injection_shift.required' => 'Shift Lot ID wajib dipilih.',
            'injection_initials.required' => 'Inisial Lot ID wajib diisi.',
            'painting_date.required' => 'Tanggal Painting wajib diisi.',
            'painting_shift.required' => 'Shift Painting wajib dipilih.',
