...
@php
    ...
    $docHeader = isset($docHeader) ? $docHeader : \App\Models\GeneralSetting::getDocHeader('sortir', $headerPlantCode, [
        'judul'      => 'LAPORAN DATA HASIL SORTIR',
        'no_dokumen' => $headerPlantCode === 'jakarta' ? 'QC-JKT-F-034/0' : 'QC-KRW-F-0208',
        'tgl_terbit' => $headerPlantCode === 'jakarta' ? '18.02.2022' : '01/01/2026',
        'revisi'     => $headerPlantCode === 'jakarta' ? '0 / 30-Dec-99' : '0',
        'halaman'    => '1/1'
    ]);
...
<head>
    <meta charset="UTF-8">
    <title>Laporan Checksheet Sortir</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 0;
...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="title">{{ $docHeader['judul'] ?? 'LAPORAN DATA HASIL SORTIR' }}</td>
            <td class="doc-info">
                <table>
        ...
        <tbody>
            @foreach($checksheets as $checksheet)
                ...
                <tr>
                    ...
                    <td style="white-space: nowrap;">{{ $checksheet->line ?? '-' }}</td>
                    <td style="white-space: nowrap;">{{ strtoupper(str_replace('_', ' ', $checksheet->source_type ?? '-')) }}</td>
                    ...
                    @if(!$isVerification)
                        ...
                        @foreach(['kashift_qc' => 'kashift_qc_time', 'supervisor_qc' => 'supervisor_qc_time', 'asst_manager_qc' => 'asst_manager_qc_time', 'manager_qc' => 'manager_qc_time'] as $field => $timeField)
                    ...
                    <td style="text-align:left; font-size:7px; min-width:70px; word-break: break-word; color: #000;">
                        @if($checksheet->next_proses)
                            <div>[{{ $checksheet->next_proses }}]</div>
                        @endif
