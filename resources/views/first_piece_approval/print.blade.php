        ...
        <tbody>
            @foreach($checksheets as $checksheet)
                ...
                <tr>
                    ...
                    {{-- Berat Part --}}
                    <td style="white-space: nowrap; font-size: 6.5px; color: #000;">
                        @php
                            $filteredWeights = array_filter($weights ?? [], fn($w) => $w !== null && $w !== '');
                        @endphp
                        @if(count($filteredWeights) > 0)
                            @foreach($filteredWeights as $ci => $wv)
                                CAV{{ $ci+1 }}: {{ $wv }}gr<br>
                            @endforeach
                        @else
                            -
