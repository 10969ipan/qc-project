    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            @if(request('view_mode') === 'verifikasi')
                <h6 class="m-0 font-weight-bold" style="color: #6f42c1;"><i class="fas fa-clipboard-check mr-1"></i> Data Hasil Verifikasi Painting</h6>
            @else
                <h6 class="m-0 font-weight-bold text-primary">Data Masuk Painting</h6>
            @endif
        </div>
        <div class="card-body">
            <form action="{{ route('painting.index') }}" method="GET" 
                class="d-flex flex-nowrap align-items-center bg-light p-2 rounded mb-4 shadow-sm"
                style="gap: 8px; overflow-x: auto; white-space: nowrap;" id="filterFormPainting">
                @if(request('plant'))
                ...
                <div class="ml-auto d-flex" style="gap: 5px;">
                    ...
                    <a href="{{ route('painting.index', ['plant' => request('plant')]) }}"
                        class="btn btn-secondary btn-sm shadow-sm rounded-pill px-3 no-loader" title="Reset Filter">
                        <i class="fas fa-undo fa-sm"></i>
                    </a>
                    @if(request('view_mode') !== 'verifikasi')
                        <a href="{{ route('painting.index', array_merge(request()->except('view_mode', 'page'), ['view_mode' => 'verifikasi', 'entry_method' => 'verification', 'plant' => request('plant')])) }}"
                    ...
                    @endif
                    <a href="{{ route('painting.daily_recap', ['start_date' => request('start_date') ?: now()->toDateString(), 'plant' => request('plant')]) }}"
                        id="btnDailyRecap"
                                ...
                                @if(request('view_mode') === 'verifikasi')
                                    <td class="align-middle">
                                        ...
                                        @php
                                            ...
                                            if ($checksheet->paintingCabutSplit && $checksheet->paintingCabutSplit->cabutRecord) {
                                                // Found the CBT split record — resolve the full chain
                                                $cabutRec = $checksheet->paintingCabutSplit->cabutRecord;
                                                $cabutQr  = $checksheet->paintingCabutSplit->generated_qrcode ?: ($isCbtFormat ? $rawQr : '-');
                                ...
                                @if(request('view_mode') === 'verifikasi' ? auth()->user()->role !== 'inspector' : !in_array(auth()->user()->role, ['inspector']))
                                    <td class="align-middle text-center text-nowrap no-export" style="min-width: 350px;">
                                        ...
                                        @if($canApproveKashift)
                                            <form
                                                action="{{ route('painting.approve', array_merge(['id' => $checksheet->id, 'type' => 'kashift'], request()->all())) }}"
                                        ...
                                        @if($canApproveSupervisor)
                                            <form
                                                action="{{ route('painting.approve', array_merge(['id' => $checksheet->id, 'type' => 'supervisor'], request()->all())) }}"
                                                method="POST" class="d-inline ajax-form">
                                                @csrf
                                        ...
                                        @if($canApproveAsst)
                                            <form
                                                action="{{ route('painting.approve', array_merge(['id' => $checksheet->id, 'type' => 'asst_manager'], request()->all())) }}"
                                                method="POST" class="d-inline ajax-form">
                                                @csrf
                                        ...
                                        @if($canApproveManager)
                                            <form
                                                action="{{ route('painting.approve', array_merge(['id' => $checksheet->id, 'type' => 'manager'], request()->all())) }}"
                                                method="POST" class="d-inline ajax-form">
                                                @csrf
                                        ...
                                        @include('partials.action_dropdown', [
                                            'canEdit'      => $showEdit,
                                            'canDelete'    => $showDel,
                                            'editUrl'      => route('painting.edit', array_merge(['id' => $checksheet->id], request()->all())),
                                            'deleteRoute'  => route('painting.destroy', array_merge(['id' => $checksheet->id], request()->all())),
                                            'deleteParams' => [],
                                            'statusUrl'    => $isAdmin ? route('painting.edit_approval', array_merge(['id' => $checksheet->id], request()->all())) : null,
                                        ])
                                        @endif
                                    </td>
                                @endif
        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Edit Checksheet Painting</h5>
            ...
            <div class="modal fade" id="rejectModal{{ $cs->id }}{{ $rejectType }}" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content border-0 shadow-lg">
                        ...
                        <form action="{{ route('painting.reject', ['id' => $cs->id, 'type' => $rejectType]) }}" method="POST">
                            @csrf
                        ...
                        <div class="col-md-3 px-2 mb-3 mb-md-0 position-relative">
                            <div class="trace-h-item step-2">
                                <div class="trace-h-header">
                                    ...
                                    <h6 class="trace-title">Painting Pasang</h6>
                                </div>
                                <div class="qr-code-box">
                        ...
                        <div class="col-md-3 px-2 mb-3 mb-md-0 position-relative">
                            <div class="trace-h-item step-3">
                                <div class="trace-h-header">
                                    ...
                                    <h6 class="trace-title">Painting Cabut</h6>
                                </div>
                                <div class="qr-code-box">
...
@push('scripts')
    ...
    <script src="{{ asset('js/checksheet/painting.js') }}"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.initPaintingIndex({
                indexRoute: "{{ route('painting.index') }}",
            ...
            if (form) {
                ...
                function syncExportLinks() {
                    var baseUrlPrint = "{{ route('painting.print') }}";
                    var baseUrlPdf = "{{ route('painting.export_pdf') }}";
                    var baseUrlRecap = "{{ route('painting.daily_recap') }}";
    ...
    @php $bulkApproveRoute = route('painting.bulk_approve'); @endphp
                ...
                btnBulkDelete.on('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: '{{ route("painting.bulk_destroy") }}',
                                type: 'POST',
                                data: {
                                    _token: '{{ csrf_token() }}',
                                    ids: selectedIds
