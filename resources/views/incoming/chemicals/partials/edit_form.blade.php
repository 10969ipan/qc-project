<form action="{{ route('incoming.chemicals.update', $checksheet->id) }}" method="POST" id="editChecksheetForm">
    @csrf @method('PUT')
    <div class="row">
        <div class="col-md-6 form-group">
            <label class="font-weight-bold">Chemical Name</label>
    ...
    <div class="row">
        ...
        <div class="col-md-3 form-group">
            <label class="font-weight-bold">Qty (Kg)</label>
            <input type="number" step="0.01" class="form-control" name="quantity_kg"
                value="{{ $checksheet->quantity_kg }}" required>
        </div>
        <div class="col-md-3 form-group">
            <label class="font-weight-bold">Komp/Jirigen (Kg)</label>
            <input type="number" step="0.01" class="form-control" name="komper_jirigen_kg"
                value="{{ $checksheet->komper_jirigen_kg }}" required>
        </div>
        <div class="col-md-3 form-group">
            <label class="font-weight-bold">Sampling (Kg)</label>
            <input type="number" step="0.01" class="form-control" name="sampling_size_jirigen_kg"
                value="{{ $checksheet->sampling_size_jirigen_kg }}" required>
        </div>
    </div>
    <div class="row">
    ...
    <div class="form-group"><label class="font-weight-bold">Remarks</label><textarea class="form-control" name="remarks"
            rows="2">{{ $checksheet->remarks }}</textarea></div>
    <div class="text-right">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
        <button type="submit" class="btn btn-primary px-4 shadow-sm">Simpan Perubahan Chemical</button>
    </div>
</form>
