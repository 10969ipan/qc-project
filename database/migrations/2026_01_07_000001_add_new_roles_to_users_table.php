<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration {
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        // Modify the enum to add new roles
        DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('admin', 'supervisor', 'inspector', 'kashift', 'asst_manager', 'manager', 'karu_qc', 'kashift_plating', 'supervisor_plating', 'manager_plating') NOT NULL DEFAULT 'inspector'");
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        // Remove the new roles from enum
        DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('admin', 'supervisor', 'inspector', 'kashift', 'asst_manager', 'manager') NOT NULL DEFAULT 'inspector'");
    }
};
